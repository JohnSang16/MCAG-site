...
export function ContainerScroll({ children, className = '', ...props }) {
  ...
  return (
    <ContainerScrollContext.Provider value={{ scrollYProgress }}>
      <div ref={scrollRef} className={`relative min-h-screen w-full ${className}`} {...props}>
        {children}
      </div>
...
export const BentoGrid = React.forwardRef(({ variant, className = '', ...props }, ref) => (
  <div ref={ref} className={bentoGridVariants({ variant, className })} {...props} />
))
...
export const ContainerScale = React.forwardRef(({ className = '', style, ...props }, ref) => {
  const { scrollYProgress } = useContainerScrollContext()
  ...
  return (
    <motion.div
      ref={ref}
      className={`left-1/2 top-1/2 size-fit ${className}`}
